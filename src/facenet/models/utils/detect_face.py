...
def fixed_batch_process(im_data, model):
	...
	return tuple(torch.cat(v, dim=0) for v in zip(*out, strict=False))
...
def detect_face(imgs, minsize, pnet, rnet, onet, threshold, factor, device):
	if isinstance(imgs, np.ndarray | torch.Tensor):
		...
		if any(img.size != imgs[0].size for img in imgs):
			raise Exception(
				'MTCNN batch processing only compatible with'
				+ ' equal-dimension images.'
			)
	...
	batch_boxes, batch_points = (
		np.array(batch_boxes, dtype=object),
		np.array(batch_points, dtype=object),
	)
...
def nms_numpy(boxes, scores, threshold, method):
	...
	while inds.size > 0:
		...
		inds = inds[np.where(o <= threshold)]
...
def rerec(bboxA):
	...
	layer = torch.max(w, h)
	bboxA[:, 0] = bboxA[:, 0] + w * 0.5 - layer * 0.5
	bboxA[:, 1] = bboxA[:, 1] + h * 0.5 - layer * 0.5
	bboxA[:, 2:4] = bboxA[:, :2] + layer.repeat(2, 1).permute(1, 0)
...
def extract_face(img, box, image_size=160, margin=0, save_path=None):
	...
	Arguments:
	    ...
	    image_size {int} -- Output image size in pixels. The image will
	            be square.
	    margin {int} -- Margin to add to bounding box, in terms of pixels
	            in the final image. Note that the application of the margin
	            differs slightly from the davidsandberg/facenet repo, which
	            applies the margin to the original image before resizing,
	            making the margin dependent on the original image size.
	    save_path {str} -- Save path for extracted face image.
	            (default: {None})
	...
	if isinstance(face, np.ndarray | Image.Image):
